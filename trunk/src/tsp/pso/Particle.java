...
class Particle implements Cloneable {
	...
	private void updateVelcity(Particle global_best, double w, double c1, double c2){
		...
		ArrayList<PSOEdge> new_velocity = new ArrayList<PSOEdge>();
		...
		PSOEdge[] global_edges = doSubAndMultiply(global_best, this, c2*Math.random());
		
		//prendi gli archi presenti nella miglior posizione locale
		PSOEdge[] local_edges = doSubAndMultiply(local_best_particle, this, 
																		  c1*Math.random());
		...
		velocity = new_velocity.toArray(new PSOEdge[]{});
	...
	//probabilità
	private static PSOEdge[] doSubAndMultiply(Particle best, Particle particle, 
																			double prob){
		
		//l'array differenza conterrà al massimo gli n archi di best
		ArrayList<PSOEdge> edges = new ArrayList<PSOEdge>();
		...
		for(Edge e : best.edges){
			...
			//aggiungi l'arco
			edges.add(new PSOEdge(prob, e));
		}
		
		return edges.toArray(new PSOEdge[]{});
	}
	...
	private static void multiply(PSOEdge[] edges, double coeff){
	...
	private static void insertEdges(PSOEdge[] src, ArrayList<PSOEdge> dst, 
